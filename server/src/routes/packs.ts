...
router.post('/cards/:cardId/contract', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    // 팀의 무료 계약 사용 횟수 확인
    const team = await pool.query('SELECT gold, free_contracts_used FROM teams WHERE id = ?', [teamId]);
    if (team.length === 0) {
      return res.status(404).json({ error: '팀을 찾을 수 없습니다' });
    }

    const freeContractsUsed = team[0].free_contracts_used || 0;
    ...
    // 팀 골드 확인 (무료가 아닌 경우만)
    if (!isFree && team[0].gold < contractCost) {
    ...
    // 골드 차감 (무료가 아닌 경우만) 및 무료 계약 횟수 증가 (무료인 경우)
    if (isFree) {
      await pool.query('UPDATE teams SET free_contracts_used = free_contracts_used + 1 WHERE id = ?', [teamId]);
    } else {
